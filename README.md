...
The live stream also shows the submitted prompt plus readable task, command, patch, search, MCP, plan, rollback, token, context compaction, rate-limit, and tool-output activity so long-running turns do not look silent while Codex is working.
...
The default stream renders the submitted prompt, assistant messages, and readable task, tool, token, context compaction, and rate-limit activity.
